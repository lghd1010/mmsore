package DTO;

public class Round1DTO {

	private int m1_idx;

	private String m1_name;

	private String m1_table_no;

	private String m1_enter;

	private String m1_comname;

	private String m1_rank;

	private String m1_1roundtime, m1_2roundtime, m1_3roundtime, m1_4roundtime;

	private String m1_1roundpoint, m1_2roundpoint, m1_3roundpoint, m1_4roundpoint;
	
	private String point, time;

	private String m1_round_ck;

	private String ent_idx;
	
	private String m1_memberno;
	
	private String m1_ma_idx;
	
	private String pointsum;
	
	private String m1_active;
	
	private String m1_cname;
	
	private String mef_idx;
	
	private String mef_name;
	
	private String mef_regdate;
	
	private String ent_birthday, ent_cname;
	
	private String b_idx;
	
	private String ent_phonnumber;
	
	private String mcn_no;
	
	private String m_birthday;
	
	private int ROWNUM;
	
	private String ma_u_event_1, ma_u_event_2, ma_u_event_3, ma_u_event_4;
	private String ma_e1_event_1, ma_e1_event_2, ma_e1_event_3, ma_e1_event_4;
	private String ma_e3_event_1, ma_e3_event_2, ma_e3_event_3, ma_e3_event_4;
	private String ma_e5_event_1, ma_e5_event_2, ma_e5_event_3, ma_e5_event_4;
	
	private String ent_name;
	private int ent_enter;


	public String getPoint() {
		return point;
	}


	public void setPoint(String point) {
		this.point = point;
	}


	public String getTime() {
		return time;
	}


	public String getEnt_phonnumber() {
		return ent_phonnumber;
	}


	public void setEnt_phonnumber(String ent_phonnumber) {
		this.ent_phonnumber = ent_phonnumber;
	}


	public void setTime(String time) {
		this.time = time;
	}


	public int getM1_idx() {
		return m1_idx;
	}


	public void setM1_idx(int m1_idx) {
		this.m1_idx = m1_idx;
	}


	public String getM1_name() {
		return m1_name;
	}


	public void setM1_name(String m1_name) {
		this.m1_name = m1_name;
	}


	public String getM1_table_no() {
		return m1_table_no;
	}


	public void setM1_table_no(String m1_table_no) {
		this.m1_table_no = m1_table_no;
	}


	public String getB_idx() {
		return b_idx;
	}


	public void setB_idx(String b_idx) {
		this.b_idx = b_idx;
	}


	public String getM1_enter() {
		return m1_enter;
	}


	public void setM1_enter(String m1_enter) {
		this.m1_enter = m1_enter;
	}


	public String getM1_comname() {
		return m1_comname;
	}


	public void setM1_comname(String m1_comname) {
		this.m1_comname = m1_comname;
	}


	public String getM1_rank() {
		return m1_rank;
	}


	public void setM1_rank(String m1_rank) {
		this.m1_rank = m1_rank;
	}


	public String getM1_1roundtime() {
		return m1_1roundtime;
	}


	public void setM1_1roundtime(String m1_1roundtime) {
		this.m1_1roundtime = m1_1roundtime;
	}


	public String getM1_2roundtime() {
		return m1_2roundtime;
	}


	public void setM1_2roundtime(String m1_2roundtime) {
		this.m1_2roundtime = m1_2roundtime;
	}


	public String getM1_3roundtime() {
		return m1_3roundtime;
	}


	public void setM1_3roundtime(String m1_3roundtime) {
		this.m1_3roundtime = m1_3roundtime;
	}


	public String getM1_4roundtime() {
		return m1_4roundtime;
	}


	public void setM1_4roundtime(String m1_4roundtime) {
		this.m1_4roundtime = m1_4roundtime;
	}


	public String getM1_1roundpoint() {
		return m1_1roundpoint;
	}


	public void setM1_1roundpoint(String m1_1roundpoint) {
		this.m1_1roundpoint = m1_1roundpoint;
	}


	public String getM1_2roundpoint() {
		return m1_2roundpoint;
	}


	public void setM1_2roundpoint(String m1_2roundpoint) {
		this.m1_2roundpoint = m1_2roundpoint;
	}


	public String getM1_3roundpoint() {
		return m1_3roundpoint;
	}


	public void setM1_3roundpoint(String m1_3roundpoint) {
		this.m1_3roundpoint = m1_3roundpoint;
	}


	public String getM1_4roundpoint() {
		return m1_4roundpoint;
	}


	public void setM1_4roundpoint(String m1_4roundpoint) {
		this.m1_4roundpoint = m1_4roundpoint;
	}


	public String getM1_round_ck() {
		return m1_round_ck;
	}


	public void setM1_round_ck(String m1_round_ck) {
		this.m1_round_ck = m1_round_ck;
	}


	public String getEnt_idx() {
		return ent_idx;
	}


	public void setEnt_idx(String ent_idx) {
		this.ent_idx = ent_idx;
	}


	public String getM1_memberno() {
		return m1_memberno;
	}


	public void setM1_memberno(String m1_memberno) {
		this.m1_memberno = m1_memberno;
	}


	public String getM1_ma_idx() {
		return m1_ma_idx;
	}


	public void setM1_ma_idx(String m1_ma_idx) {
		this.m1_ma_idx = m1_ma_idx;
	}


	public String getPointsum() {
		return pointsum;
	}


	public void setPointsum(String pointsum) {
		this.pointsum = pointsum;
	}


	public String getM1_active() {
		return m1_active;
	}


	public void setM1_active(String m1_active) {
		this.m1_active = m1_active;
	}


	public String getM1_cname() {
		return m1_cname;
	}


	public void setM1_cname(String m1_cname) {
		this.m1_cname = m1_cname;
	}


	public String getMef_idx() {
		return mef_idx;
	}


	public void setMef_idx(String mef_idx) {
		this.mef_idx = mef_idx;
	}


	public String getMef_name() {
		return mef_name;
	}


	public void setMef_name(String mef_name) {
		this.mef_name = mef_name;
	}


	public String getMef_regdate() {
		return mef_regdate;
	}


	public void setMef_regdate(String mef_regdate) {
		this.mef_regdate = mef_regdate;
	}


	public String getEnt_birthday() {
		return ent_birthday;
	}


	public void setEnt_birthday(String ent_birthday) {
		this.ent_birthday = ent_birthday;
	}


	public String getEnt_cname() {
		return ent_cname;
	}


	public void setEnt_cname(String ent_cname) {
		this.ent_cname = ent_cname;
	}


	public String getMcn_no() {
		return mcn_no;
	}


	public void setMcn_no(String mcn_no) {
		this.mcn_no = mcn_no;
	}


	public String getM_birthday() {
		return m_birthday;
	}


	public void setM_birthday(String m_birthday) {
		this.m_birthday = m_birthday;
	}


	public int getROWNUM() {
		return ROWNUM;
	}


	public void setROWNUM(int rOWNUM) {
		ROWNUM = rOWNUM;
	}


	public String getEnt_name() {
		return ent_name;
	}


	public void setEnt_name(String ent_name) {
		this.ent_name = ent_name;
	}


	public int getEnt_enter() {
		return ent_enter;
	}


	public void setEnt_enter(int ent_enter) {
		this.ent_enter = ent_enter;
	}


	public String getMa_u_event_1() {
		return ma_u_event_1;
	}


	public void setMa_u_event_1(String ma_u_event_1) {
		this.ma_u_event_1 = ma_u_event_1;
	}


	public String getMa_u_event_2() {
		return ma_u_event_2;
	}


	public void setMa_u_event_2(String ma_u_event_2) {
		this.ma_u_event_2 = ma_u_event_2;
	}


	public String getMa_u_event_3() {
		return ma_u_event_3;
	}


	public void setMa_u_event_3(String ma_u_event_3) {
		this.ma_u_event_3 = ma_u_event_3;
	}


	public String getMa_u_event_4() {
		return ma_u_event_4;
	}


	public void setMa_u_event_4(String ma_u_event_4) {
		this.ma_u_event_4 = ma_u_event_4;
	}


	public String getMa_e1_event_1() {
		return ma_e1_event_1;
	}


	public void setMa_e1_event_1(String ma_e1_event_1) {
		this.ma_e1_event_1 = ma_e1_event_1;
	}


	public String getMa_e1_event_2() {
		return ma_e1_event_2;
	}


	public void setMa_e1_event_2(String ma_e1_event_2) {
		this.ma_e1_event_2 = ma_e1_event_2;
	}


	public String getMa_e1_event_3() {
		return ma_e1_event_3;
	}


	public void setMa_e1_event_3(String ma_e1_event_3) {
		this.ma_e1_event_3 = ma_e1_event_3;
	}


	public String getMa_e1_event_4() {
		return ma_e1_event_4;
	}


	public void setMa_e1_event_4(String ma_e1_event_4) {
		this.ma_e1_event_4 = ma_e1_event_4;
	}


	public String getMa_e3_event_1() {
		return ma_e3_event_1;
	}


	public void setMa_e3_event_1(String ma_e3_event_1) {
		this.ma_e3_event_1 = ma_e3_event_1;
	}


	public String getMa_e3_event_2() {
		return ma_e3_event_2;
	}


	public void setMa_e3_event_2(String ma_e3_event_2) {
		this.ma_e3_event_2 = ma_e3_event_2;
	}


	public String getMa_e3_event_3() {
		return ma_e3_event_3;
	}


	public void setMa_e3_event_3(String ma_e3_event_3) {
		this.ma_e3_event_3 = ma_e3_event_3;
	}


	public String getMa_e3_event_4() {
		return ma_e3_event_4;
	}


	public void setMa_e3_event_4(String ma_e3_event_4) {
		this.ma_e3_event_4 = ma_e3_event_4;
	}


	public String getMa_e5_event_1() {
		return ma_e5_event_1;
	}


	public void setMa_e5_event_1(String ma_e5_event_1) {
		this.ma_e5_event_1 = ma_e5_event_1;
	}


	public String getMa_e5_event_2() {
		return ma_e5_event_2;
	}


	public void setMa_e5_event_2(String ma_e5_event_2) {
		this.ma_e5_event_2 = ma_e5_event_2;
	}


	public String getMa_e5_event_3() {
		return ma_e5_event_3;
	}


	public void setMa_e5_event_3(String ma_e5_event_3) {
		this.ma_e5_event_3 = ma_e5_event_3;
	}


	public String getMa_e5_event_4() {
		return ma_e5_event_4;
	}


	public void setMa_e5_event_4(String ma_e5_event_4) {
		this.ma_e5_event_4 = ma_e5_event_4;
	}
	

}